...
- standardize `project.arch.yaml`
- expand docs/examples/e2e coverage

### Definition of done

- `init` and `plan` are reproducible in local and CI contexts
- `project.arch.yaml` is the default contract source for planning and verification
...
## Active phase issues (Phase6-Phase9)
...
Archflow は最初からすべてを解決しようとすべきではありません。
...
Phase 0 は、コントリビューターと将来のユーザーが Archflow が何をしようとしているかを理解するために必要な最小限の構造を作成します。
...
Archflow の最小限の安定した概念モデルを定義します。

#### このフェーズが重要な理由

安定した概念モデルなしには、実装がずれます。
Phase 1 は Archflow に語彙と内部アーキテクチャを与えます。
...
この段階で、Archflow は概念ドキュメントから実際の運用ツールへと移行すべきです。
...
ここで Archflow は汎用スキャフォルドツールとは明らかに異なるものになります。
...
- AI 支援開発における Archflow の価値が見えている
...
Archflow を実際のプロジェクトへの採用を容易にし、スタイル間でより再利用可能にします。
...
- Archflow が現実的なプロジェクトへの採用が容易に感じられる
...
Archflow はコードを実行するだけでなく、ファイルを読むことで理解できるべきです。
...
Archflow は Rust に適した examples から始めることができますが、Rust 専用になってはいけません。
...
ロードマップは Archflow が焦点を維持するのを助けるべきです。
...
**Archflow は層ごとに成長すべきです：まず明確さ、次に構造、次に実行、そして再利用。**
...
- `init` / `plan` の再現性を強化
- `project.arch.yaml` を契約の正本として標準化
