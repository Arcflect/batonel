...
Phase 4 (Verification) introduced the first operational `archflow verify` command.
...
- [ADR-0007](./0007-archflow-remains-sidecar-first-before-code-aware-analysis.md): sidecar-first before code-aware analysis
...
The following checks are explicitly included in the first version of `archflow verify`.
...
|---|---|
| `root-file-exists` | Required archflow config files exist (`project.arch.yaml`, `placement.rules.yaml`, `artifacts.plan.yaml`, `contracts.template.yaml`) |
...
[ADR-0007](./0007-archflow-remains-sidecar-first-before-code-aware-analysis.md).
All checks operate on archflow files, config files, and file system structure,
...
- if the check touches only archflow files or file system structure, it may fit Phase 4
...
Phase 4 (Verification) において、最初の `archflow verify` コマンドが実装されました。
...
以下のチェックが初版の `archflow verify` に明示的に含まれます。
...
| check_id | 説明 |
|---|---|
| `root-file-exists` | 必須 archflow 設定ファイルが存在すること |
