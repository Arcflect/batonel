...
At this stage, presets must remain an extension of existing Archflow assets,
...
Preset-packaged files consumed by `archflow init --preset` must remain compatible
with current schema assumptions for root config files:

- `project.arch.yaml` -> project schema expectations
...
- Presets stay understandable as an extension of the current Archflow model.
...
この段階では、preset は既存 Archflow 資産の拡張であるべきで、
...
`archflow init --preset` で利用される preset パッケージファイルは、
現在の root config 用 schema 前提と互換でなければなりません。

- `project.arch.yaml` -> project schema 前提
...
- preset は現在の Archflow モデルの拡張として理解しやすくなる。
