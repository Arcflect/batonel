...
This directory contains architectural and product decisions for Batonel.

The purpose of these records is to preserve important reasoning over time.

Batonel includes many design choices that may otherwise become unclear later, such as:
...
- how much Batonel should depend on code-aware analysis
...
- `0001-batonel-is-an-architecture-to-execution-bridge.md`
...
The initial set of decisions focuses on core Batonel direction:
...
This directory helps Batonel preserve intent over time.
...
このディレクトリには、Batonel のアーキテクチャおよびプロダクトに関する決定事項が含まれています。

これらの記録の目的は、重要な reasoning を時間をかけて保全することです。

Batonel には、後になって不明確になる可能性のある多くの設計上の選択が含まれています。例えば：

- コアコンセプトが何を意味するか
- 何が真実の源とみなされるか
- prompt が contract とどのように関係するか
- examples が preset とどのように関係するか
- Batonel がコード認識分析にどの程度依存すべきか
- verify がどのように始まるべきか
...
初期の決定事項は Batonel のコアな方向性に焦点を当てています。
...
- `0007` Batonel はコード認識分析の前に sidecar ファースト のままである
...
このディレクトリは Batonel が意図を時間をかけて保全するのを助けます。
