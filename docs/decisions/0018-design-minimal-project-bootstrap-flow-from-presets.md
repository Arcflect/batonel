...
This must align with existing `batonel init` behavior and stay minimal.

## Decision

`batonel init` becomes the single bootstrap entrypoint for both:
...
1. user runs `batonel init --preset <preset-id>`
...
- `--project-name <name>`: overrides `project.name` in `project.baton.yaml`
...
- `batonel init` remains the canonical start command.
...
この設計は既存の `batonel init` と整合しつつ、最小である必要があります。

## 決定事項

`batonel init` を次の単一ブートストラップ入口にします。
...
1. ユーザーが `batonel init --preset <preset-id>` を実行
...
- `--project-name <name>`: `project.baton.yaml` の `project.name` を上書き
...
- `batonel init` は正規の開始コマンドとして維持される。
