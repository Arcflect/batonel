...
Batonel is expected to include a `verify` capability in the future.
...
Batonel’s current maturity is centered on:
...
Batonel verification begins with **structure and contract consistency**.
...
- keeping verification aligned with the current Batonel model
...
Because Batonel starts from architecture rather than code,
...
Batonel は将来的に `verify` 機能を含むことが期待されています。
...
Batonel の verify は**構造と contract の整合**から始まります。
...
- 現在の Batonel モデルと整合した verify を保つこと
...
Batonel はコードではなくアーキテクチャから始まるため、最初の verify レイヤーもコードではなくアーキテクチャから始まるべきです。
