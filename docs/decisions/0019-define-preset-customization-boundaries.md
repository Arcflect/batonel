...
Batonel's model requires the opposite:
...
- `project.baton.yaml`: project name, module set, feature naming
...
- changes should remain verifiable through existing `batonel verify` checks
...
- Preset philosophy remains aligned with Batonel's non-dogmatic architecture stance.
...
- `project.baton.yaml`: project 名、module セット、feature 命名
...
- 変更は既存の `batonel verify` チェックで検証可能な状態を保つ
...
- preset 哲学は Batonel の非ドグマ的な設計姿勢と整合したまま維持される。
