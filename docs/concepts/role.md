...
Archflow における **Role** は、artifact のアーキテクチャ上の分類です。
...
ロールにより Archflow は artifact の名前を知らなくても、artifact がプロジェクト構造のどこに属するかを知ることができます。
...
ロールがなければ、Archflow はアーキテクチャ上の意味なしに名前のリストだけを持つことになります。
...
ロールの命名はプロジェクトに依存します。
Archflow 自体は固定されたロールセットを強制しません。
