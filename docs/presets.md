...
This document explains what a **preset** means in Batonel,
...
In Batonel, a **preset** is a reusable configuration package
...
Batonel is designed to be flexible.
...
Batonel should not assume that there is one correct architecture.
...
This is a core Batonel principle:
...
- how Batonel can express different architectural styles
...
- `package.compatibility.*`: compatibility gates for batonel/schema support
...
For the first preset packaging model, Batonel should use one self-contained directory per preset.

The expected repository shape is:

- `presets/<preset-name>/preset.yaml`
- `presets/<preset-name>/project.baton.yaml`
...
Batonel Phase 8 introduces a prototype local registry workflow.

- verify: `batonel preset-verify --preset-dir presets/<preset-id>` — check alignment before publishing
- publish: `batonel preset-publish --preset-dir presets/<preset-id> --registry-dir .batonel/registry`
- install: `batonel preset-install --preset <preset-id> --registry-dir .batonel/registry --destination-dir presets`

Registry index format (prototype):

- `.batonel/registry/index.yaml`
- `.batonel/registry/packages/<preset-id>/<version>/...`
...
The `preset-verify` command checks that presets remain anchored to Batonel's core philosophy and do not collapse into simple directory templates without architectural meaning.
...
```bash
batonel preset-verify --preset-dir presets/generic-layered
```
...
- `project.baton.yaml` carries project defaults
...
Batonel Phase 9 introduces versioned preset migration tooling to support controlled upgrades while preserving architecture guarantees.
...
- Before overwriting any existing file, Batonel backs it up to `.batonel/migration-backup/<from-version>/` so upgrades are fully reversible

```bash
batonel preset-migration-apply \
  --preset generic-layered \
  --from-version 0.1.0 \
  --to-version 0.2.0 \
  --registry-dir .batonel/registry
...
Preset-based startup should be aligned with `batonel init`.

Minimal flow:

1. run `batonel init --preset <preset-id>`
2. generate root config files from `presets/<preset-id>/`
3. keep existing files untouched (skip if already present)
4. continue with `batonel plan` and `batonel scaffold`
...
Batonel currently includes these examples:
...
Current role:
- the smallest example for understanding the Batonel model
...
To reduce ambiguity, Batonel uses the following canonical mapping:
...
- schema expectations for root config files consumed by `batonel init --preset`
...
The preset becomes an officially supported Batonel starting package.
...
This is important because Batonel is meant to preserve intent,
...
Each such directory should carry its own `preset.yaml` and standard Batonel files.
...
At the current stage of Batonel, the repository does not need a full preset system yet.
...
A preset in Batonel is a reusable starting point for architecture-aware scaffolding.
...
このドキュメントは Batonel における **preset** の意味、preset と examples の関係、そして現在の examples が将来的に再利用可能な preset へと進化する方法を説明します。
...
Batonel における **preset** は、一般的なアーキテクチャスタイルまたはエコシステムのための再利用可能な設定パッケージです。
...
Batonel は 1 つの正しいアーキテクチャがあると仮定すべきではありません。
...
- Batonel が異なるアーキテクチャスタイルをどのように表現できるか
...
Batonel には現在これらの examples が含まれています。
...
現在の役割：
- Batonel モデルを理解するための最小の example
...
曖昧さを減らすため、Batonel は次の正式マッピングを使います。
...
Preset が正式にサポートされた Batonel の出発点パッケージになります。
...
これは Batonel が意図を保全するためのものであり、1 つの固定された構造を強制するためではないため重要です。
...
- `project.baton.yaml`: project 名、module 名、feature 命名
...
最初の preset packaging モデルでは、Batonel は preset ごとに 1 つの自己完結したディレクトリを使うべきです。
...
- `project.baton.yaml` は project defaults を持つ
...
preset ベースの開始フローは `batonel init` と整合させます。

最小フロー:

1. `batonel init --preset <preset-id>` を実行
2. `presets/<preset-id>/` から root config ファイルを生成
3. 既存ファイルは変更せず（存在時は skip）
4. `batonel plan` と `batonel scaffold` を続けて実行
...
それぞれのディレクトリは、自身の `preset.yaml` と標準 Batonel ファイルを持つべきです。
...
Batonel の現段階では、リポジトリにはまだ完全な preset システムは必要ありません。
...
Batonel における preset は、アーキテクチャを意識したスキャフォルディングのための再利用可能な出発点です。
