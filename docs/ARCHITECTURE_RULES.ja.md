...
本ドキュメントは、Archflow におけるアーキテクチャおよびコーディング上の必須ルールを定義するものです。

Archflow は CLI ツールですが、明確な境界を持つアプリケーションとして扱わなければなりません。
...
Archflow は単なる薄い CLI ラッパーではありません。
Archflow は、CLI を通して提供される「アーキテクチャ判断エンジン」です。
...
Archflow のコードは、以下のレイヤに分離しなければなりません。
...
- Archflow において最も重要な業務ロジックを含まなければならない
...
Archflow の機能は、技術的仕組みではなく、概念を中心にモデリングしなければならない。
...
Archflow は `project.arch.yaml` のようなプロジェクト設定ファイルを読む可能性が高い。
...
Archflow は text, markdown, json など複数の出力形式を持ちうる。
...
Archflow は判断ツールであるため、テストは必須である。
...
Archflow の判断の中心に近いロジックほど、integration test のみに依存してはならない。
...
Archflow は、アーキテクチャ思考のためのツールである。
