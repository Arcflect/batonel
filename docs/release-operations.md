...
3. Update `Cargo.toml` version to match the draft version:
...
5. In GitHub → Releases, open the draft and click **Publish release**.
   - GitHub creates the release tag automatically.
   - Two CI workflows trigger on `release: published`:
...
> Update `Cargo.toml` **before** publishing the release. If the versions differ, `verify-tag-version.yml` will fail and no binaries will be attached.
