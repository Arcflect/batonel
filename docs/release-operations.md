...
Version is validated by `verify-tag-version.yml` when the tag is pushed.
...
2. Confirm the draft release version in GitHub → Releases.

3. Update `Cargo.toml` version to match the resolved version:
...
6. CI runs automatically:
   - `verify-tag-version.yml`: validates `Cargo.toml` version matches the tag
   - `archflow-release-cli.yml`: builds binaries, runs smoke tests, uploads assets to the release-drafter draft, and publishes the release

> **Important**: `archflow --version` output is determined by `Cargo.toml` at compile time. The tag must match the `Cargo.toml` version; `verify-tag-version.yml` enforces this on every tag push.
...
|---|---|---|
| `release-drafter.yml` | push to `main` | Updates draft release body from merged PR titles/labels |
...
| `verify-tag-version.yml` | push tag `v*.*.*` | Validates `Cargo.toml` version matches the pushed tag |
| `archflow-release-cli.yml` | push tag `v*.*.*` | Builds binaries, smoke tests, uploads assets, publishes release |
