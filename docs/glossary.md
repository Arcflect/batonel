...
This glossary defines the core terms used in Batonel.
...
A project describes the overall frame in which Batonel interprets modules,
...
The smallest planned implementation unit in Batonel.
...
Artifacts are the main units Batonel scaffolds, constrains, and hands off
...
Placement rules help Batonel determine where an artifact should live.
...
It is one of the main inputs Batonel uses for scaffold generation.
...
Contracts are the main way Batonel preserves architectural intent
...
The generated structural output produced by Batonel.
...
remain consistent with Batonel rules.
...
In Batonel, AI handoff is based on:
...
In Batonel, sidecar files commonly include:

- `*.contract.yaml`
- `*.prompt.md`

Sidecar files are important because they allow Batonel to work
...
A short way to describe what Batonel is.

It means Batonel sits between:
...
Batonel does not stop at documentation.
...
この用語集は Batonel で使用されるコアな用語を定義します。
...
ロールは Batonel が次のことを判断するのに役立ちます。
...
配置ルールは Batonel が artifact がどこに配置されるべきかを判断するのに役立ちます。
...
これは Batonel がスキャフォルド生成に使用する主要な入力の 1 つです。
...
プロジェクト構造と artifact の定義が Batonel のルールと整合して一致したままかどうかをチェックするプロセス。
...
Batonel では、sidecar ファイルには一般的に次のものが含まれます。

- `*.contract.yaml`
- `*.prompt.md`

Sidecar ファイルは、本番コードが存在する前でも Batonel が機能できるため重要です。
...
Batonel が何であるかを簡潔に表現した言葉。

Batonel が次の間に位置することを意味します。
