...
This document explains how Batonel works as a flow from architectural intent
to implementation scaffolding and AI handoff.

The goal of this document is to show:

- what Batonel takes as input
- how Batonel interprets that input
- what Batonel produces as output
...
At a high level, Batonel follows this sequence:
...
Batonel begins from structured input files.
...
1. `batonel init --preset <preset-id> --project-name <name>`
2. optional preview: `batonel init ... --dry-run`
3. `batonel plan`
...
Without an artifact plan, Batonel has no execution units to work with.
...
Once the input files are available, Batonel interprets them in layers.

### Step 1. Read project context

Batonel first reads the project definition.

This establishes the global context in which all later interpretation happens.

At this stage, Batonel identifies:
...
Next, Batonel loads placement rules and creates a role-to-path map.

At this stage, Batonel can answer:
...
Then Batonel loads the artifact list.

At this stage, Batonel knows the concrete implementation units that should exist.
...
Using placement rules and artifact roles, Batonel resolves where each artifact should live.
...
At this stage, Batonel can determine the expected scaffold location of every artifact.

### Step 5. Apply contract templates

If contract templates exist, Batonel applies role-based defaults to each artifact.
...
Batonel then turns artifact identity + resolved path + template defaults
...
From each artifact contract, Batonel generates AI handoff prompts.
...
Batonel can then scaffold the project structure.
...
Batonel produces outputs in several layers.
...
The internal dependency chain of Batonel looks like this:
...
Batonel is focused on the flow in between.
...
Batonel’s flow is built on several principles.
...
Batonel works by turning architecture into progressively more executable forms.
...
**Batonel does not start from code.
...
このドキュメントは、Batonel がアーキテクチャの意図から実装の骨組みおよび AI へのハンドオフへと至るフローとして、どのように機能するかを説明します。

このドキュメントの目的は、次の点を示すことです。

- Batonel が入力として何を受け取るか
- Batonel がその入力をどのように解釈するか
- Batonel が出力として何を生成するか
...
Artifact プランがなければ、Batonel には対応する実行ユニットがありません。
...
1. `batonel init --preset <preset-id> --project-name <name>`
2. 必要に応じて `batonel init ... --dry-run` で事前確認
3. `batonel plan`

この導線は、ローカルとCIの両方で再現可能であることを重視しています。

入力ファイルが揃ったら、Batonel はそれらを層ごとに解釈します。

#### Step 1. プロジェクトコンテキストを読み込む

Batonel はまずプロジェクト定義を読み込みます。

これにより、その後のすべての解釈が行われるグローバルコンテキストが確立されます。

この段階で Batonel は以下を特定します。
...
次に、Batonel は配置ルールを読み込み、ロール-パスのマップを作成します。

この段階で Batonel が答えられることは次のとおりです。
...
次に Batonel は artifact リストを読み込みます。

この段階で Batonel は、存在すべき具体的な実装ユニットを把握します。
...
配置ルールと artifact のロールを使用して、Batonel は各 artifact がどこに配置されるべきかを解決します。
...
contract テンプレートが存在する場合、Batonel は各 artifact にロールベースのデフォルトを適用します。
...
Batonel は artifact のアイデンティティ + 解決されたパス + テンプレートのデフォルトを、artifact 固有の contract に変換します。
...
各 artifact contract から、Batonel は AI ハンドオフ prompt を生成します。
...
Batonel は、アーキテクチャを段階的により実行可能な形式に変換することで機能します。
